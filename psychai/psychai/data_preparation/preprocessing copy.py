...
class DataPreprocessor:
    ...
    def prepare_training_data_given_X_y(self, X, y, X_columns, y_column, column_to_split="", correlation_threshold = 0.05,
                                        explained_variance_threshold=0.9, 
                     remove_redundant_features=True, remove_low_variance=True, 
                     variance_threshold=0.01, use_pca=False, outlier_strategies = ['Replace with Median'], random_state= 1):
        """
  ...
  # Define a function to handle outliers with a parameter k, ensuring columns are numerical
    ...
    def prepare_training_data(self, X_columns, y_column, column_to_split="", explained_variance_threshold=0.95, 
        ...
        if len(outlier_strategies)>0:
            for outlier_strategy in outlier_strategies:
                if outlier_strategy == 'Remove Multi-Column Outlier':
                    ...
                    if (m<3):
                        m = 3
                    self.df = self.remove_rows_with_m_outliers(self.df.copy(), m=m, k=k)
                ...
                if outlier_strategy == 'Replace with Median':
                    self.df = self.replace_outliers_with_median(self.df.copy(), k=k)
                    print("Replace with Median")
                if outlier_strategy == 'Replace with Similar Median':
                    similarity_df = self.calculate_cosine_similarity(self.df)
                    self.df = self.replace_outliers_with_most_similar_median(self.df.copy(), similarity_df, k=k)
                    print(f"Replace with Similar Median. k={k}")      
                if outlier_strategy == 'Replace with Boundary':
                    self.df = self.replace_outliers_with_bounds(self.df, k=1.5)
                    print(f"Replace with Boundary. k={k}")      


        # Step 1: Extracting Features and Target Labels
        X = self.df[X_columns].copy()  # Extract features as DataFrame
        y = self.df[y_column].values   # Extract target variable as numpy array

        return self.prepare_training_data_given_X_y(X,y,X_columns, y_column, column_to_split = column_to_split, 
